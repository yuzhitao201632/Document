...
def ml_lost():
	...
	for user in role_detail_dict.values():
		...
		value_list=[]
		for attr in attr_list:
			tmp_value=ratio_array[seq]
			if user.is_lost:
				if stat_lost_attr_dict.has_key(attr):
					stat_lost_attr_dict[attr].append(tmp_value)
				else:
					tmp_attr_list=[]
					tmp_attr_list.append(tmp_value)
					stat_lost_attr_dict[attr]=tmp_attr_list
			#非丢失
			else:
				if stat_unlost_attr_dict.has_key(attr):
					stat_unlost_attr_dict[attr].append(tmp_value)
				else:
					tmp_attr_list=[]
					tmp_attr_list.append(tmp_value)
					stat_unlost_attr_dict[attr]=tmp_attr_list
			seq+=1
	...
	print "lost_rate=%f:all user num=%d:create_role_dict=%d"%(lost_sum/float(all_detail_user),all_detail_user,create_role_dict.__len__())
	seq=0;
	for attr in stat_unlost_attr_dict.keys():
		nas=np.array(stat_lost_attr_dict[attr])
		nau=np.array(stat_unlost_attr_dict[attr])
		print  "stat prop %s lost mean=%f:std=%f: unlost mean=%f:std=%f"%(attr,nas.mean(),nas.std(),nau.mean(),nau.std())
